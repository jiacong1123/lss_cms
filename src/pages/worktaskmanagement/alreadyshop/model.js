...
import * as alreadyshopApi from './service';

export default {
  namespace: 'alreadyshop',
  state: {
    loading: false,    // 加载动画
  ...
  effects: {
    ...
    * EFFECTS_GET_CURRENTORDER({payload}, { call, put , select}){
      ...
      const { result, obj, msg } = yield call(alreadyshopApi.getOrderDetail, payload);
    ...
    * EFFECTS_JUDGE_USERISEXIST({payload}, { call, put , select}){
      const { result, obj, msg } = yield call(alreadyshopApi.judgeUserIsExist, payload);
      if (result === 1 ) {
        if (obj) {
    ...
    * EFFECTS_GET_SEARCHVALUE({payload}, { call, put , select}){
      const searchValue = yield select(({alreadyshop}) => alreadyshop.searchValue)
      yield put({
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        // 路由切换初始化数据
        if ( pathname === '/worktaskmanagement/alreadyshop'){
          dispatch({
