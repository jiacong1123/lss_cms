...
import * as waitfollowupApi from './service';

export default {
  namespace: 'waitfollowup',
  state: {
    loading: false,    // 加载动画
  ...
  effects: {
    ...
    * EFFECTS_GET_SOURCRCHILD({payload}, { call, put }) {
      yield put(_mmAction('IS_SHOWLOADING',{loading: true}))
      const {tagid} = payload
      const { result , obj , msg} = yield call(waitfollowupApi.getSourceChild, {tagid, ...payload});
      if ( result === 1) {
    ...
    * EFFECTS_SET_SOURCECHILDNAME({payload}, { call, put , select}){
      const sourceChildList = yield select(({waitfollowup}) => waitfollowup.jobordersourcechild)
    ...
    * EFFECTS_GET_CURRENTORDER({payload}, { call, put , select}){
      ...
      const { result, obj, msg } = yield call(waitfollowupApi.getOrderDetail, payload);
    ...
    * EFFECTS_JUDGE_USERISEXIST({payload}, { call, put , select}){
      const { result, obj, msg } = yield call(waitfollowupApi.judgeUserIsExist, payload);
      if (result === 1 ) {
        if (obj) {
    ...
    * EFFECTS_GET_SEARCHVALUE({payload}, { call, put , select}){
      const searchValue = yield select(({waitfollowup}) => waitfollowup.searchValue)
      yield put({
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        // 路由切换初始化数据
        if ( pathname === '/worktaskmanagement/waitfollowup'){
          dispatch({
