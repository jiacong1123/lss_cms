...
import * as newsinformationApi from './service';
...
export default {
  ...
  effects: {
    ...
    * EFFECTS_GET_NEWSLIST({payload}, { call, put,select }) {
      ...
      const searchValue = yield select(({newsinformation}) => newsinformation.searchValue)
      const currentPage = yield select(({newsinformation}) => newsinformation.currentPage)
      const { page } = payload
      const { result, obj , total, msg  } = yield call(newsinformationApi.getNewsList, {...searchValue,...payload})
      if (result === 1) {
    ...
    * EFFECTS_SAVE_NEWS({payload}, { call, put, select}) {
      yield put(_mmAction('IS_SHOWLOADING',{loading: true}))
      const currentPage = yield select(({newsinformation}) => newsinformation.currentPage)
      const { result, msg  } = yield call(newsinformationApi.saveNews, payload);
      if (result === 1) {
    ...
    * EFFECTS_GET_CURRENTNEWS({payload}, { call, put , select}){
      const newslist = yield select(({newsinformation}) => newsinformation.newslist)
      const { id } = payload
      const currentNews = newslist.find(item=>item.id === id )
      yield put({
        type: 'GET_CURRENTNEWS',
        payload: {
          id,
          currentNews
        }
    ...
    * EFFECTS_OPER_NEWS({payload}, { call, put , select}){
      yield put(_mmAction('IS_SHOWLOADING',{loading: true}))
      const currentPage = yield select(({newsinformation}) => newsinformation.currentPage)
      const { result, msg  } = yield call(newsinformationApi.operNews, payload);
      if (result === 1) {
        yield put(_mmAction('EFFECTS_GET_NEWSLIST',{
          page:currentPage,
          limit: 10
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        // 路由切换初始化数据
        if ( pathname === '/operatemanagement/newsinformation'){
          dispatch({
